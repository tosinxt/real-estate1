...
type CharProps = { label: string; index: number };
function Char({ label, index }: CharProps) {
  return (
    <span
      data-label={label}
...
export function JoinButton({
  ...
  const chars1 = label1.split("");
  const chars2 = label2.split("");
        ...
        <div className="wrap">
          ...
          <div className="content">
            <span className="char state-1">
              {chars1.map((ch, i) => (
                <Char key={i} label={ch === " " ? " " : ch} index={i + 1} />
              ))}
            </span>
            <div className="icon">
              <div />
